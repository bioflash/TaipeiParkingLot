                ...
                request(requestObjForMap, (err, res, body)=>{
                    if (!err){
                        try{
                            ...
                            log.info(`Succesfully got map data for ${key} from ${parkingConfig[key]['parkingLotMapURL']}, updating to local repository`);
                            fs.writeFileSync(path.join(__dirname, '../datasets/'+key), body);
                        }catch(e){
                            log.warn(`Failed to got map data for ${key} from ${parkingConfig[key]['parkingLotMapURL']}, read from local instead`);
...
function fetchRealtimeParkingData(key, resolve, reject){
    ...
    request(requestObjForAvailParking, (err, res, body)=>{
        try{
            if (err) throw commons.wrapError(e,`Unable to get available parking in ${key}`);
            //The execution of availHandler can cause error
            try{
                AVAIL_PARKING[key] = parkingConfig[key]['availHandler'](body)  
                if (typeof resolve==='function') resolve()
            }catch(e){
                AVAIL_PARKING[key]=[];
                throw commons.wrapError(e, 'Error while invoking availHandler callback function');
            }
        }catch(e){
...
function startRealtimeParkingUpdateSchedular(){
    setInterval(()=>Object.keys(parkingConfig).forEach((key)=>{
        fetchRealtimeParkingData(key, ()=> log.info(`The schedular updated current available parking lot info for ${key}`))        
    }),REFRESH_TIME_FRAME)
}
